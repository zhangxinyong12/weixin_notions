#!/usr/bin/env python3
"""微信公众号 HTML 合规校验器（zxy skill 自带，确定性检查微信兼容性）。

把微信编辑器的"必须遵守的平台限制"从模型自觉变成确定性兜底。
排版生成后必跑：检查禁用标签/属性/样式，并核查文字节点是否用
<span leaf=""> 包裹（公众号编辑器粘贴后保持样式的关键）。

用法:
    validate_html.py <file.html>
    validate_html.py --stdin < file.html
...
# Windows 控制台默认 GBK，emoji/中文输出会 UnicodeEncodeError —— 强制 UTF-8
